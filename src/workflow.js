...
async function workflow(repo) {
  ...
  try {
    ...
    if (keys.includes("update:webdriver")) {
      const update = { name: "update:webdriver", run: "npm run update:webdriver -- --versions.chrome 83.0.4103.116" };
    ...
    if (keys.includes("start:webdriver")) {
      const start = { name: "start:webdriver", run: "npm run start:webdriver -- --versions.chrome 83.0.4103.116 &" };
